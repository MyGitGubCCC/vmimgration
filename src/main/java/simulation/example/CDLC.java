...
import simulation.vmShedulePolicy.VmSheduleHostCDLC;
...
public class CDLC {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        VmSheduleHost vmSheduleHost = new VmSheduleHostCDLC();
        // 初始化任务选择策略
        ...
        for (Vm vm : vmList) {
            ...
            if(host != null) {
                ...
                //System.out.println("虚拟机" + vm.getId() + "放置在主机" + host.getId() +"上");
            }
            else {
                mipsRequest += vm.getMips();
                //System.out.println("虚拟机" + vm.getId() + "目前没有合适的主机放置");
            }
        ...
        while (true){
            ...
            if (cloudletListQueue.size() != 0 ){
                ...
                migMessage = vmSheduleHost.getVmMigrationHost(
                        hostList, starSimTime, migEnergy,
                        mign,mipsRequest, mipsAllcation,vmList);
        ...
        System.out.println("虚拟机请求mips:" + mipsAllcation);
        System.out.println("虚拟机实际分配mips:" +mipsRequest);
